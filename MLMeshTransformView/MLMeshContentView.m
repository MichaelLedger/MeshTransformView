...
- (void)ML_recursivelyExecuteBlock:(void (^)(CALayer *layer))block;
@end
...
- (void)ML_recursivelyExecuteBlock:(void (^)(CALayer *layer))block
{
    ...
    for (CALayer *layer in self.sublayers) {
        [layer ML_recursivelyExecuteBlock:block];
    }
}

@end

@interface MLMeshContentView()
@property (nonatomic, strong) NSMutableSet *observedLayers;
@end

@implementation MLMeshContentView
...
- (void)registerAsObserverToLayer:(CALayer *)layer
{
    ...
    for (NSString *key in [MLMeshContentView observedKeys]) {
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(CALayer *)layer change:(NSDictionary *)change context:(void *)context
{
    ...
    [self.layer ML_recursivelyExecuteBlock:^(CALayer *layer) {

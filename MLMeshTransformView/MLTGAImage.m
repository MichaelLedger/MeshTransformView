...
- (instancetype)initWithTGAFileAtLocation:(NSURL *)location
{
    ...
    if(self)
    {
        ...
        {
            // Metal是不能理解一个24-BPP格式的图像，所以我们必须转化成TGA数据
            // 从24比特BGA格式到32比特BGRA格式(类似MTLPixelFormatBGRA8Unorm)
            NSMutableData *mutableData = [[NSMutableData alloc] initWithLength:dataSize];
            
            // TGA规范，图像数据是在标题和ID之后立即设置指针到文件的开头+头的大小+ID的大小
            // 初始化源指针，源代码数据为BGR格式

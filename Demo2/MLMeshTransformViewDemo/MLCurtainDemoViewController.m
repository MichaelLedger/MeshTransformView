...
@interface MLCurtainDemoViewController () <UIGestureRecognizerDelegate>
@property (nonatomic) BOOL drags;
@property (nonatomic) CGFloat surplus;
@end

@implementation MLCurtainDemoViewController
...
- (void)pan:(UIPanGestureRecognizer *)sender
{
    ...
    switch (sender.state) {
        ...
        case UIGestureRecognizerStateChanged:
            self.transformView.meshTransform = [MLMutableMeshTransform curtainMeshTransformAtPoint:CGPointMake(point.x + self.surplus, point.y)
...
{
    self.transformView.meshTransform = [MLMutableMeshTransform curtainMeshTransformAtPoint:CGPointMake(self.view.bounds.size.width, self.view.bounds.size.height/2.0)
                                             boundsSize:self.transformView.bounds.size];
}
